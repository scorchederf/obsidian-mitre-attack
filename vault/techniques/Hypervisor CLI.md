...
aliases: 
    - T1059.012
mitre-attack: https://attack.mitre.org/techniques/T1059/012
tactic: 
     - Execution
platforms:
     - ESXi
...
Adversaries may abuse hypervisor command line interpreters (CLIs) to execute malicious commands. Hypervisor CLIs typically enable a wide variety of functionality for managing both the hypervisor itself and the guest virtual machines it hosts. <br><br>For example, on ESXi systems, tools such as `esxcli` and `vim-cmd` allow administrators to configure firewall rules and log forwarding on the hypervisor, list virtual machines, start and stop virtual machines, and more.[^291] [^149] [^180]  Adversaries may be able to leverage these tools in order to support further actions, such as [File and Directory Discovery](https://attack.mitre.org/techniques/T1083) or [Data Encrypted for Impact](https://attack.mitre.org/techniques/T1486).


### Procedure Examples
| ID | Name | Use |
| --- | --- | --- |
| [[Cheerscrypt\|S1096]] | Cheerscrypt | Cheerscrypt has leveraged `esxcli` in order to terminate running virtual machines.[^60] <br> |
| [[VIRTUALPIE\|S1218]] | VIRTUALPIE | [VIRTUALPIE](https://attack.mitre.org/software/S1218) is capable of command line execution on compromised ESXi servers.[^157]  |
| [[Royal\|S1073]] | Royal | Royal ransomware uses `esxcli` to gather a list of running VMs and terminate them.[^162]  |
| [[UNC3886\|G1048]] | UNC3886 | [UNC3886](https://attack.mitre.org/groups/G1048) has used the esxcli command line utility to modify firewall rules, install malware, and for artifact removal.[^157] [^305]  |
...
[^60]: [Trend Micro Cheerscrypt May 2022](https://www.trendmicro.com/en_se/research/22/e/new-linux-based-ransomware-cheerscrypt-targets-exsi-devices.html)
...
[^162]: [Trend Micro Royal Linux ESXi February 2023](https://www.trendmicro.com/en_us/research/23/b/royal-ransomware-expands-attacks-by-targeting-linux-esxi-servers.html)
...
[^180]: [LOLESXi](https://lolesxi-project.github.io/LOLESXi/)
...
[^291]: [Broadcom ESXCLI Reference](https://developer.broadcom.com/xapis/esxcli-command-reference/latest/)
...
[^305]: [Google Cloud Threat Intelligence VMWare ESXi Zero-Day 2023](https://cloud.google.com/blog/topics/threat-intelligence/vmware-esxi-zero-day-bypass/)
